...
# 1. 페이지 설정 및 반응형 최적화
st.set_page_config(page_title="조별과제 잔혹사: 나의 팀플 성향 & 착각 진단", page_icon="👥", layout="centered")

# 2. 아기자기하고 귀여운 파스텔톤 팀플 감성 UI 디자인 (CSS)
st.markdown("""
    <style>
...
# 👥 조별과제 100% 특화 질문 데이터셋 (학술적 이론 명칭 매칭)
questions = [
    {
        "id": "낙관성 편향",
        "badge": "📢 마감 직전의 빌런 발생",
...
# --- 🏠 화면 0: 첫 인트로 페이지 (조별과제 컨셉 개편) ---
if st.session_state.step == 0:
    st.write("")
    st.markdown("<div class='main-title'>👥 조별과제 잔혹사:<br>나의 팀플 성향 & 착각 진단 테스트</div>", unsafe_allow_html=True)
    st.markdown("<div class='sub-title'>“과연 나는 팀플에서 얼마나 이성적이고 합리적일까?”<br><span style='font-size:1.05rem; color:#A08E81;'>잠수 빌런, 단톡방 안읽씹 상황으로 알아보는 나의 인지 오류 테스트</span></div>", unsafe_allow_html=True)
    ...
    <div class='card-container' style='text-align: center;'>
        ...
        <b>나를 꼭 닮은 조별과제 동물 캐릭터 유형과 최고의 팀플 파트너</b>를 확인해 보세요!
        </p>
    </div>
    """, unsafe_allow_html=True)
    
    if st.button("👉 나의 팀플 유형 진단하기 (Start)", use_container_width=True):
...
# --- 📝 화면 1 ~ 6: 질문 스텝 ---
    ...
    st.caption(f"🐾 팀플 마감일까지 {st.session_state.step} / 6 문항 진행 중")
    ...
    st.write("")
    if st.button("고르고 다음 회의로 넘어가기 ➡️", use_container_width=True):
...
# --- 📊 화면 7: 종합 분석 대시보드 ---
    ...
    st.markdown("<div class='main-title'>📊 팀플 비합리성 진단 리포트</div>", unsafe_allow_html=True)
    st.write("")
    ...
    st.markdown(f"<h2 style='text-align:center; margin-bottom: 25px; color:#4A3E3D;'>나의 팀플 착각 지수: <span style='color:#FF6B6B;'>{final_avg:.0f}점</span></h2>", unsafe_allow_html=True)
    
    st.markdown("---")
    st.markdown("### 🏹 나의 조별과제 행동 유형 캐릭터")
    
    # 🦫 조별과제 세계관 동물 캐릭터 + 찰떡 케미 가이드 완벽 통합
    if bias_count <= 1:
        st.markdown("""
        <div class='type-container' style='border-left: 6px solid #4A5568;'>
            <div class='character-avatar'>🦅</div>
            <h4 style='color:#2D3748;'>유형 1: 칼같고 이성적인 팀플 매크로, '팩트 독수리'</h4>
            ...
            <p style='font-size: 1.1rem !important; color: #2D3748;'><b>🤝 같이 조 짜면 대박 나는 환상의 파트너:</b></p>
        ...
        <div class='type-container' style='border-left: 6px solid #319795;'>
            <div class='character-avatar'>🦫</div>
            <h4 style='color:#234E52;'>유형 2: 묵묵히 제 몫을 다하는 평화주의자, '실무 비버'</h4>
            ...
            <hr style='border: 1px dashed #E2E8F0; margin: 15px 0;'>
            <p style='font-size: 1.1rem !important; color: #234E52;'><b>🤝 같이 조 짜면 대박 나는 환상의 파트너:</b></p>
            <p style='font-size: 1.05rem !important; margin-left: 10px;'>
        ...
        <div class='type-container' style='border-left: 6px solid #ED8936;'>
            <div class='character-avatar'>🐰</div>
            <h4 style='color:#7B341E;'>유형 3: 눈치 보며 속앓이하는 감성 요정, '불안 보스 토끼'</h4>
            ...
            <hr style='border: 1px dashed #E2E8F0; margin: 15px 0;'>
            <p style='font-size: 1.1rem !important; color: #7B341E;'><b>🤝 같이 조 짜면 대박 나는 환상의 파트너:</b></p>
            <p style='font-size: 1.05rem !important; margin-left: 10px;'>
        ...
        <div class='type-container' style='border-left: 6px solid #E53E3E;'>
            <div class='character-avatar'>🦖</div>
            <h4 style='color:#9B2C2C;'>유형 4: 다 비켜! 내가 혼자 다 한다, '폭주 공룡(리더렉스)'</h4>
            ...
            <hr style='border: 1px dashed #E2E8F0; margin: 15px 0;'>
            <p style='font-size: 1.1rem !important; color: #9B2C2C;'><b>🤝 같이 조 짜면 대박 나는 환상의 파트너:</b></p>
            <p style='font-size: 1.05rem !important; margin-left: 10px;'>
    ...
    <div class='solution-box'>
        ...
        <p class='solution-desc'>
        단톡방 안읽씹이 이어질 때 에타의 빌런 썰들을 기억하며 '나 왕따 시키나?' 소설을 쓰지 마세요. <b>'내 친한 친구가 이 상황이라면 내가 뭐라고 해줄까?'</b>라고 제3자 시선으로 생각해보면, 그저 다들 시험 기간이거나 알바 중이라는 아주 보편적인 상식적 이유가 보입니다.
        </p>
    </div>
    """, unsafe_allow_html=True)
