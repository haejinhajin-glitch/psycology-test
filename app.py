...
questions = [
    ...
    },
    {
        ...
        "options": ["인스타 피드만 화려해 보일 뿐, 남들과 똑같이 취업 걱정하고 조원 잔혹사에 시달리는 평범한 학생이다.", "엄청난 금수저 집안이면서 동시에 지능도 천재적이고 팀원 복까지 타고나 스트레스를 아예 모르는 완벽한 존재다."]
    },
    {
        "id": "전망 이론",
...
if st.session_state.step == 0:
    ...
    <div class='card-container' style='text-align: center;'>
        ...
        <b>나를 꼭 닮은 조별과제 동물 캐릭터 유형</b>을 확인해 보세요!
        </p>
    </div>
    """, unsafe_allow_html=True)
...
# --- 📊 화면 7: 종합 분석 대시보드 ---
    ...
    if st.session_state.user_selections.get("전망 이론") == "景리스크형: '이판사판이다!' 동전을 던지듯 도박하는 심정으로, 교수님께 고발해 잘되면 가산점을 받고 잘못 꼬이면 조 전체가 감점되는 리스크를 감수한다.":
    ...
    if bias_count <= 1:
        ...
        <div class='type-container' style='border-left: 6px solid #E53E3E;'>
            ...
            <p><b>⚠️ 타인이 나를 볼 때 주의할 점:</b> 추진력은 시원시원하지만, 팀원들 눈에는 '조원들의 피드백을 무시하고 혼자 고집 부리며 폭주하는 독재자'로 보일 위험이 큽니다. 잘못된 방향인데도 미련 때문에 밀어붙이다가 다 같이 침몰할 수 있으니 늘 나침반(팀원 의견)을 확인하세요.</p>
        </div>
        """, unsafe_allow_html=True)
